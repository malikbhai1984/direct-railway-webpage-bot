...
// Top Leagues IDs (API-Football) - EXPANDED for Arab Cup
const TOP_LEAGUES = {
...
async function fetchFromApiFootball() {
  try {
    ...
    console.log('📅 Fetching for:', today, 'and', tomorrow);
    ...
    for (const targetDate of [today, tomorrow]) {
      for (const [leagueId, leagueName] of Object.entries(TOP_LEAGUES)) {
        try {
          ...
          if (apiFootballCalls >= API_FOOTBALL_LIMIT) {
            console.log('⚠️ API limit reached, stopping fetch');
            break;
          }
        } catch (error) {
...
app.get('/api/matches', async (req, res) => {
  try {
    ...
    // FIXED: Only return matches that are NOT finished
    const matches = await Match.find({
      status: { $nin: ['FT', 'AET', 'PEN'] } // Exclude finished matches
    })
      .sort({ match_date: 1 }) // Sort by date ascending (soonest first)
      .limit(100);
    
    console.log(`📊 Active matches (non-finished): ${matches.length}`);
...
setInterval(async () => {
  ...
  try {
    ...
    // STEP 1: Clean up finished matches (older than 2 hours)
    const twoHoursAgo = new Date(Date.now() - 2 * 60 * 60 * 1000);
    const deletedMatches = await Match.deleteMany({
      status: { $in: ['FT', 'AET', 'PEN'] },
      match_date: { $lt: twoHoursAgo }
    });
    
    if (deletedMatches.deletedCount > 0) {
      console.log(`🗑️ Cleaned up ${deletedMatches.deletedCount} finished matches`);
    }
    
    // STEP 2: Update predictions for active matches only
    const matches = await Match.find({
